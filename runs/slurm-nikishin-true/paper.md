...
tool_version: 0.1.0
run_at: 2026-05-14T21:17:31.639786+00:00
...
$$\min _ { \theta } \mathbb { E } _ { s , a , s ^ { \prime } } \left [ \| f _ { \theta } ( s , a ) - s ^ { \prime } \| ^ { 2 } \right ] , \\ \min _ { \theta } \mathbb { E } _ { s , a , r } \left [ ( r _ { \theta } ( s , a ) - r ) ^ { 2 } \right ] .$$
...
$$\text {the model. The optimization problem becomes
} & \quad \max \, J ( \pi _ { Q } ) \\ & \quad Q , \theta \\ & \quad s . t . \ Q ( s , a ) = B ^ { \theta } Q ( s , a ) \ \forall s \in \mathcal { S } , a \in \mathcal { A } , \\ & \quad \text {where } \pi _ { Q } ( a | s ) = \frac { \exp Q ( s , a ) } { \sum _ { a ^ { \prime } } \exp Q ( s , a ^ { \prime } ) } . \\ B ^ { \theta } \text { here is the soft Bellman optimality operator with re-}$$

B θ here is the soft Bellman optimality operator with respect to the model parameters θ :

$$\stackrel { \text {ds} } { h a s } \quad & B ^ { \theta } Q ( s , a ) \equiv r _ { \theta } ( s , a ) + \gamma \mathbb { E } _ { p _ { \theta } ( s ^ { \prime } | s , a ) } \log \sum _ { a ^ { \prime } } \exp Q ( s ^ { \prime } , a ^ { \prime } ) . \\ \text {f a } \quad & W _ { \ } a b o d o r \quad \text {the} \quad \text {soft} \quad \text {Rollmon} \quad \text {oporctor} \quad \text {with} \quad$$
...
$$\theta \stackrel { \varphi } { \rightarrow } Q ^ { * } \stackrel { \exp } { \longrightarrow } \pi _ { Q ^ { * } } \stackrel { \ a c t } { \longrightarrow } J .$$
...
$$\frac { \partial J ( \theta ) } { \partial \theta } = \underbrace { \frac { \partial J ( \pi ) } { \partial \pi } } _ { P G } \cdot \underbrace { \frac { \partial \pi ( Q ^ { * } ) } { \partial Q ^ { * } } } _ { I F T } \cdot \underbrace { \frac { \partial \varphi ( \theta ) } { \partial \theta } } _ { I F } \cdot$$
...
$$\min _ { Q , \theta } L ^ { \text {true} } ( Q ) & \triangle q \sum _ { s , a } ( Q ( s , a ) - B Q ( s , a ) ) ^ { 2 } , \\ s . t . \ Q ( s , a ) & = B ^ { \theta } Q ( s , a ) \ \forall s \in \mathcal { S } , a \in \mathcal { A } ,$$
...
$$\theta = \begin{cases} \frac { \kappa } { \| \theta \| } \theta & \text {if } \| \theta \| > \kappa , \\ \theta & \text {if } \| \theta \| \leq \kappa \end{cases} .$$

Finding an MLE solution corresponds to minimizing the average KL divergence

$$\overline { D _ { K L } } ( p | | p _ { \theta } ) = \frac { 1 } { | \mathcal { S } | \cdot | \mathcal { A } | } \sum _ { s , a , s ^ { \prime } } p ( s ^ { \prime } | s , a ) \log \frac { p ( s ^ { \prime } | s , a ) } { p _ { \theta } ( s ^ { \prime } | s , a ) }$$
...
$$B ^ { \theta } Q ^ { * } ( s , a ) = B ^ { \theta ^ { \prime } } Q ^ { * } ( s , a ) \ \forall s \in \mathcal { S } , a \in \mathcal { A } . \quad ( 9 )$$
...
$$\lim _ { \alpha \to 0 } \alpha \log \sum _ { a ^ { \prime } } \exp \frac { 1 } { \alpha } Q ( s ^ { \prime } , a ^ { \prime } ) = \max _ { a ^ { \prime } } Q ( s ^ { \prime } , a ^ { \prime } ) . \quad ( 1 0 )$$
...
$$\max _ { s , a } \left | Q ^ { * } ( s , a ) - \hat { Q } _ { M L E } ( s , a ) \right | \leq \frac { \epsilon _ { r } } { 1 - \gamma } + \frac { \gamma \epsilon _ { p } r _ { \max } } { 2 ( 1 - \gamma ) ^ { 2 } } ;$$
...
$$\max _ { s , a } \left | Q ^ { * } ( s , a ) - \hat { Q } _ { O M D } ( s , a ) \right | \leq \frac { \epsilon } { 1 - \gamma } .$$
...
$$\min _ { w } L ( \theta , w ) \triangle q \min _ { w } \mathbb { E } _ { s , a } [ Q _ { w } ( s , a ) - B ^ { \theta } Q _ { \bar { w } } ( s , a ) ] ^ { 2 } , \ ( 1 1 )$$
...
$$\frac { \partial L ( \theta , w ) } { \partial w } = 0 .$$
...
$$L ^ { \text {true} } ( w ) \triangle q \mathbb { E } _ { s , a } [ Q _ { w } ( s , a ) - B Q _ { \bar { w } } ( s , a ) ] ^ { 2 } , \quad ( 1 3 )$$

where B , again, is a soft Bellman operator induced by the true reward r and dynamics p :

$$B Q _ { \bar { w } } ( s , a ) \triangle q r ( s , a ) + \gamma \mathbb { E } _ { p ( s ^ { \prime } | s , a ) } \log \sum _ { a ^ { \prime } } \exp Q _ { \bar { w } } ( s ^ { \prime } , a ^ { \prime } ) .$$
...
$$\frac { \partial L ^ { \text {true} } ( \theta ) } { \partial \theta } \approx - \underbrace { \frac { \partial L ^ { \text {true} } ( w ^ { * } ) } { \partial w } } _ { \text {grad Bellman} } \cdot \underbrace { \frac { \partial ^ { 2 } L ( \theta , w ^ { * } ) } { \partial \theta \partial w } } _ { \text {approx IFT} } \Big | _ { w ^ { * } = \varphi ( \theta ) } \quad ( 1 4 )$$
...
$$\ell _ { V E P } ( \theta ) = \sum _ { \pi \in \Pi } \sum _ { V \in \mathcal { V } } \sum _ { s \in \mathcal { S } } \left ( B _ { \pi } V ( s ) - B _ { \pi } ^ { \theta } V ( s ) \right ) ^ { 2 } , \quad ( 1 5 ) \quad \stackrel { \text {return} } { \quad } \quad \stackrel { \text {tup} } { \quad } \quad \stackrel { \text {tup} } { \quad } \quad \stackrel { \text {tup} } { \quad } \quad \stackrel { \text {tup} } { \quad } \quad \stackrel { \text {tup} } { \quad } \quad \stackrel { \text {tup} } { \quad }$$
